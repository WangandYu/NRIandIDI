...
get_probs = function(X0,delta0, Z0, t0){
  ...
  for(i in 1:5){
    ...
    cox.haz1 = matrix(rep(cox.basehaz1$hazard,each=length(ids[[i]])),length(ids[[i]]),dim.uniquetime)	# baseline cumulative hazard matrix for cause 1
    cox.haz1 = cox.haz1*matrix(rep(exp(cbind(y1,y2,y4)%*%cox1$coef),dim.uniquetime),length(ids[[i]]),dim.uniquetime)
    # calcualte the cumulative cause specific hazard for cause 2 #
    cox.haz2 = matrix(rep(cox.basehaz2$hazard,each=length(ids[[i]])),length(ids[[i]]),dim.uniquetime)	# baseline cumulative hazard matrix for cause 2
    cox.haz2 = cox.haz2*matrix(rep(exp(cbind(y1,y2,y4)%*%cox2$coef),dim.uniquetime),length(ids[[i]]),dim.uniquetime)
    ...
    # Then calculate the comulative baseline hazard fuction for cause 2 #
    # data2 = data.frame(cbind(X, event=ifelse(delta==2,1,0), z1=z1, z2=z2, z3=z3, z4=z4, z5=z5))
    cox2 = coxph(Surv(X, event) ~ z1  + z2+ z3+z4+z5, data2)	
    #summary(cox2)
    cox.basehaz2 = basehaz(cox2, centered=FALSE)
    ...
    # estimate the cumulative cause specific hazard for cause 1 #
    cox.haz1 = matrix(rep(cox.basehaz1$hazard,each=length(ids[[i]])),length(ids[[i]]),dim.uniquetime)	# baseline cumulative hazard matrix for cause 1
    cox.haz1 = cox.haz1*matrix(rep(exp(cbind(y1,y2,y3,y4,y5)%*%cox1$coef),dim.uniquetime),length(ids[[i]]),dim.uniquetime)
    
    # calcualte the cumulative cause specific hazard for cause 2 #
    cox.haz2 = matrix(rep(cox.basehaz2$hazard,each=length(ids[[i]])),length(ids[[i]]),dim.uniquetime)	# baseline cumulative hazard matrix for cause 2
    cox.haz2 = cox.haz2*matrix(rep(exp(cbind(y1,y2,y3,y4,y5)%*%cox2$coef),dim.uniquetime),length(ids[[i]]),dim.uniquetime)
...
estNRI_Weibull = function(X,delta, Z, t0, withsd)
{
  ...
  # save(res,file = 'E:Legacy_Cox_probs.rda')
  p1.hat.m1=res[[1]]
  p2.hat.m1=res[[2]]
  p3.hat.m1=res[[3]]
  p1.hat.m2=res[[4]]
...
gene=function(s){
  ...
  t0 = c(8,10,12)
  return(estNRI_Weibull(DATA.boot$t,DATA.boot$indicator,DATA.boot[,4:9], t0, FALSE))
}
library(parallel)
...
clusterExport(clust, c("estNRI_Weibull","coxph","Surv","basehaz","get_probs"))
