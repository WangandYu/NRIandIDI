...
Final=dt=read.csv("C:\\Users\\Zander Wang\\Box\\ZhengWang\\MACS\\Legacy\\survivaldata.csv")
X=dt$t
delta=dt$indicator
ndim=nrow(dt)
library(survival)
...
load('E:Legacy_Cox_probs.rda')
R1=res[[4]]

w1=ifelse((Final$t<=8)&(Final$indicator>0),1,0)/est.S.censoring+ifelse(Final$t>8,1,0)/Cen2[1]
mean((ifelse((Final$t<=8)&Final$ind1,1,0)-R1[,1])^2*w1)
w1=ifelse((Final$t<=10)&(Final$indicator>0),1,0)/est.S.censoring+ifelse(Final$t>10,1,0)/Cen2[2]
mean((ifelse((Final$t<=10)&Final$ind1,1,0)-R1[,2])^2*w1)
w1=ifelse((Final$t<=12)&(Final$indicator>0),1,0)/est.S.censoring+ifelse(Final$t>12,1,0)/Cen2[3]
mean((ifelse((Final$t<=12)&Final$ind1,1,0)-R1[,3])^2*w1)

load('E:Legacy_Fine_probs.rda')
R2=res[[4]]
w1=ifelse((Final$t<=8)&(Final$indicator>0),1,0)/est.S.censoring+ifelse(Final$t>8,1,0)/Cen2[1]
...
out1 <- diag_crr(Crsk(t,ind1)~AGE+CESD+LEU3N+COHORT+LEU3N2,data=Final,test="lin",seed=2020)
print(out1)
out2 <- diag_crr(Crsk(t,ind1)~AGE+CESD+LEU3N+COHORT+LEU3N2,data=Final,test="prop",seed=2020)
print(out2)
out3 <- diag_crr(Crsk(t,ind2)~AGE+CESD+LEU3N+COHORT+LEU3N2,data=Final,test="lin",seed=2020)
print(out3)
out4 <- diag_crr(Crsk(t,ind2)~AGE+CESD+LEU3N+COHORT+LEU3N2,data=Final,test="prop",seed=2020)
print(out4)
