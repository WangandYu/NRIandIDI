...
for(m in 1:nsim)
{
  ...
  for(t0 in T0){
    ...
    write.table(t(c(n1,n2,n3)),paste("IDI_n_",t0,".txt",sep=""), sep="\t", row.names = FALSE,col.names = FALSE,append = TRUE)
  }
  #############################################################################
  ...
  for(i in 1:3){
    ...
    write.table(t(c(sum1,sum2,sum3)),paste("IDI_sum_",T0[i],".txt",sep=""), sep="\t", row.names = FALSE,col.names = FALSE,append = TRUE)
  }
}


dir2=''
ns = read.table(paste(dir2,"IDI_n_20.txt",sep=""),header = FALSE)
dim(ns)
ns = apply(ns,2,sum)
totaln = sum(ns)

sums = read.table(paste(dir2,"IDI_sum_20.txt",sep=""),header = FALSE)
dim(sums)
sums = apply(sums,2,sum)

sum(sums/ns/(1-ns/totaln))/3

ns = read.table(paste(dir2,"IDI_n_21.txt",sep=""),header = FALSE)
dim(ns)
ns = apply(ns,2,sum)
totaln = sum(ns)

sums = read.table(paste(dir2,"IDI_sum_21.txt",sep=""),header = FALSE)
dim(sums)
sums = apply(sums,2,sum)

sum(sums/ns/(1-ns/totaln))/3

ns = read.table(paste(dir2,"IDI_n_22.txt",sep=""),header = FALSE)
dim(ns)
ns = apply(ns,2,sum)
totaln = sum(ns)

sums = read.table(paste(dir2,"IDI_sum_22.txt",sep=""),header = FALSE)
dim(sums)
sums = apply(sums,2,sum)
