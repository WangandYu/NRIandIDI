...
wrapper <- function(name){
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      boot.IDI2 = na.omit(boot.IDI)
      sd.IDI = t(apply(boot.IDI2,2,sd))
...
#################################################################################
nSim <- 1000
IDIFineUsingFine400_Alternative <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(IDIFineUsingFine400_Alternative,file = 'IDIFineUsingFine400_Alternative.rda')
