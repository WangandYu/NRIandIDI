...
wrapper <- function(name){
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      cov = as.numeric((NRI-qnorm(0.975)*NRI_sd/sqrt(ndim))<c(0.2090376,0.1886032,0.1688827) & (NRI+qnorm(0.975)*NRI_sd/sqrt(ndim))>c(0.2090376,0.1886032,0.1688827))
      ...
      cov1=ifelse(quantile(boot.NRI[,1],alpha1[1])<0.2090376 & quantile(boot.NRI[,1],alpha2[1])>0.2090376,1,0)
      cov2=ifelse(quantile(boot.NRI[,2],alpha1[2])<0.1886032 & quantile(boot.NRI[,2],alpha2[2])>0.1886032,1,0)
      cov3=ifelse(quantile(boot.NRI[,3],alpha1[3])<0.1688827 & quantile(boot.NRI[,3],alpha2[3])>0.1688827,1,0)
      cov10=ifelse(quantile(boot.NRI[,1],0.025)<0.2090376 & quantile(boot.NRI[,1],0.975)>0.2090376,1,0)
      cov20=ifelse(quantile(boot.NRI[,2],0.025)<0.1886032 & quantile(boot.NRI[,2],0.975)>0.1886032,1,0)
      cov30=ifelse(quantile(boot.NRI[,3],0.025)<0.1688827 & quantile(boot.NRI[,3],0.975)>0.1688827,1,0)
...
#################################################################################
nSim <- 1000
NRIGerdsUsingCox200_Alternative <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIGerdsUsingCox200_Alternative,file = 'NRIGerdsUsingCox200_Alternative.rda')
