...
wrapper <- function(name){
  estNRI_Weibull = function(X,delta, z1, z2,z3, t0, withsd)
  {ndim = length(X)
  npts = length(t0)
  ...
  estNRI_Weibull_unnest = function(X,delta, z1, z2,z3, t0, withsd)
  {ndim = length(X)
  npts = length(t0)
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      ests=try(estNRI_Weibull(X,delta,z1, z2,z3, t0, TRUE),silent=TRUE)
      ...
      for(i in 1:nboot)
      {
        ...
        z2.boot = z2[index]
        z3.boot = z3[index]
        zanshi=try(estNRI_Weibull(X.boot,delta.boot,z1.boot, z2.boot,z3.boot, t0, FALSE),silent=TRUE)
      ...
      before=rbind(NRI,NRI_sd,cov,sd.NRI,c(cov1,cov2,cov3),c(cov10,cov20,cov30))
      
      
      ests=try(estNRI_Weibull_unnest(X,delta,z1, z2,z3, t0, TRUE),silent=TRUE)
      if(class(ests)=="try-error") return(NULL)
      NRI = t(ests[,1])
      NRI_sd = t(ests[,2])
      ...
      for(i in 1:nboot)
      {
        ...
        z2.boot = z2[index]
        z3.boot = z3[index]
        zanshi=try(estNRI_Weibull_unnest(X.boot,delta.boot,z1.boot, z2.boot,z3.boot, t0, FALSE),silent=TRUE)
        if(class(zanshi)=="try-error") next
        boot.NRI = rbind(boot.NRI,t(zanshi))
      }
      jack.NRI = NULL
      ...
      after=rbind(NRI,NRI_sd,cov,sd.NRI,c(cov1,cov2,cov3),c(cov10,cov20,cov30))
...
#################################################################################
nSim <- 1000
NRIGerdsUsingCox200_NULL <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIGerdsUsingCox200_NULL,file = 'NRIGerdsUsingCox200_NULL.rda')
