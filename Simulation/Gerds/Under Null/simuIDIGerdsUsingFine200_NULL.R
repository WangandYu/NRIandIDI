...
wrapper <- function(name){
  ...
  estIDI_Fine_unnest = function(X,delta, z1, z2,z3, t0, withsd)
  {ndim = length(X)
  npts = length(t0)
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      ests=try(estIDI_Fine_unnest(X,delta,z1, z2,z3, t0, FALSE),silent=TRUE)
      if(class(ests)=="try-error") return(NULL)
      IDI = t(ests)
      ...
      for(i in 1:nboot)
      {
        ...
        z2.boot = z2[index]
        z3.boot = z3[index]
        zanshi=try(estIDI_Fine_unnest(X.boot,delta.boot,z1.boot, z2.boot,z3.boot, t0, FALSE),silent=TRUE)
        if(class(zanshi)=="try-error") next
        boot.IDI = rbind(boot.IDI,t(zanshi))
      }
      jack.IDI = NULL
...
#################################################################################
nSim <- 1000
IDIGerdsUsingFine200_NULL <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(IDIGerdsUsingFine200_NULL,file = 'IDIGerdsUsingFine200_NULL.rda')
