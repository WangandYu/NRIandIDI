...
wrapper <- function(name){
  ...
  estIDI_Weibull_unnest = function(X,delta, z1, z2,z3, t0, withsd)
  {ndim = length(X)
  npts = length(t0)
  ...
  # calcualte the cumulative baseline hazard function for cause 1 #
  data1 = data.frame(cbind(X, event=ifelse(delta==1,1,0), z1=z1,z2=z2,z4=z4))
  names(data1) = c("X","event","z1","z2","z4")
  ...
  # Then calculate the comulative baseline hazard fuction for cause 2 #
  data2 = data.frame(cbind(X, event=ifelse(delta==2,1,0), z1=z1,z2=z2,z4=z4))
  names(data2) = c("X","event","z1","z2","z4")
  ...
  # estimate the cumulative cause specific hazard for cause 1 #
  cox.haz1 = matrix(rep(cox.basehaz1[['hazard']],each=ndim),ndim,dim.uniquetime)	# baseline cumulative hazard matrix for cause 1
  cox.haz1 = cox.haz1*matrix(rep(exp(cbind(z1,z2,z4)%*%coef(cox1)),dim.uniquetime),ndim,dim.uniquetime)
  # calcualte the cumulative cause specific hazard for cause 2 #
  cox.haz2 = matrix(rep(cox.basehaz2[['hazard']],each=ndim),ndim,dim.uniquetime)	# baseline cumulative hazard matrix for cause 2
  cox.haz2 = cox.haz2*matrix(rep(exp(cbind(z1,z2,z4)%*%coef(cox2)),dim.uniquetime),ndim,dim.uniquetime)
  ...
  # calcualte the cumulative baseline hazard function for cause 1 #
  data1 = data.frame(cbind(X, event=ifelse(delta==1,1,0), z1=z1, z2=z2,z3=z3,z5=z5))
  names(data1) = c("X","event","z1","z2","z3","z5")
  cox1 = coxph(Surv(X, event) ~ z1 + z2+z3+z5, data1)	
  #summary(cox1)
  cox.basehaz1 = basehaz(cox1, centered=FALSE)
  cox.basehaz1 = cox.basehaz1[cox.basehaz1[['time']]<=t0[npts],]	# only keep the baseline hazard until time t0
  
  # Then calculate the comulative baseline hazard fuction for cause 2 #
  data2 = data.frame(cbind(X, event=ifelse(delta==2,1,0), z1=z1, z2=z2,z3=z3,z5=z5))
  names(data2) = c("X","event","z1","z2","z3","z5")
  cox2 = coxph(Surv(X, event) ~ z1 + z2+z3+z5, data2)	
  #summary(cox2)
  cox.basehaz2 = basehaz(cox2, centered=FALSE)
  cox.basehaz2 = cox.basehaz2[cox.basehaz2[['time']]<=t0[npts],]	# only keep the baseline hazard until time t0
  ...
  # estimate the cumulative cause specific hazard for cause 1 #
  cox.haz1 = matrix(rep(cox.basehaz1[['hazard']],each=ndim),ndim,dim.uniquetime)	# baseline cumulative hazard matrix for cause 1
  cox.haz1 = cox.haz1*matrix(rep(exp(cbind(z1,z2,z3,z5)%*%coef(cox1)),dim.uniquetime),ndim,dim.uniquetime)
  
  # calcualte the cumulative cause specific hazard for cause 2 #
  cox.haz2 = matrix(rep(cox.basehaz2[['hazard']],each=ndim),ndim,dim.uniquetime)	# baseline cumulative hazard matrix for cause 2
  cox.haz2 = cox.haz2*matrix(rep(exp(cbind(z1,z2,z3,z5)%*%coef(cox2)),dim.uniquetime),ndim,dim.uniquetime)
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      beta=c(3,1)
      t=(log((1/runif(ndim)-1)/2)-cbind(z1,z2)%*%beta)
      ...
      cov1=ifelse(quantile(boot.IDI[,1],alpha1[1])<0 & quantile(boot.IDI[,1],alpha2[1])>0,1,0)
      cov2=ifelse(quantile(boot.IDI[,2],alpha1[2])<0 & quantile(boot.IDI[,2],alpha2[2])>0,1,0)
      cov3=ifelse(quantile(boot.IDI[,3],alpha1[3])<0 & quantile(boot.IDI[,3],alpha2[3])>0,1,0)
      cov10=ifelse(quantile(boot.IDI[,1],0.025)<0 & quantile(boot.IDI[,1],0.975)>0,1,0)
      cov20=ifelse(quantile(boot.IDI[,2],0.025)<0 & quantile(boot.IDI[,2],0.975)>0,1,0)
      cov30=ifelse(quantile(boot.IDI[,3],0.025)<0 & quantile(boot.IDI[,3],0.975)>0,1,0)
      before=rbind(IDI,sd.IDI,c(cov1,cov2,cov3),c(cov10,cov20,cov30))
      
      
      ests=try(estIDI_Weibull_unnest(X,delta,z1, z2,z3, t0, FALSE),silent=TRUE)
      if(class(ests)=="try-error") return(NULL)
      IDI = t(ests)
      ...
      for(i in 1:nboot)
      {
        ...
        z2.boot = z2[index]
        z3.boot = z3[index]
        zanshi=try(estIDI_Weibull_unnest(X.boot,delta.boot,z1.boot, z2.boot,z3.boot, t0, FALSE),silent=TRUE)
        if(class(zanshi)=="try-error") next
        boot.IDI = rbind(boot.IDI,t(zanshi))
      }
      jack.IDI = NULL
      ...
      cov20=ifelse(quantile(boot.IDI[,2],0.025)<0 & quantile(boot.IDI[,2],0.975)>0,1,0)
      cov30=ifelse(quantile(boot.IDI[,3],0.025)<0 & quantile(boot.IDI[,3],0.975)>0,1,0)
      after=rbind(IDI,sd.IDI,c(cov1,cov2,cov3),c(cov10,cov20,cov30))
...
#################################################################################
nSim <- 1000
IDIGerdsUsingCox200_NULL <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(IDIGerdsUsingCox200_NULL,file = 'IDIGerdsUsingCox200_NULL.rda')
