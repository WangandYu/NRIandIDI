...
wrapper <- function(name){
  ...
  estNRI_Fine_unnest = function(X,delta, z1, z2,z3, t0, withsd)
  {ndim = length(X)
  npts = length(t0)
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      ests=try(estNRI_Fine_unnest(X,delta,z1, z2,z3, t0, TRUE),silent=TRUE)
      if(class(ests)=="try-error") return(NULL)
      NRI = t(ests[,1])
      NRI_sd = t(ests[,2])
      ...
      for(i in 1:nboot)
      {
        ...
        z2.boot = z2[index]
        z3.boot = z3[index]
        zanshi=try(estNRI_Fine_unnest(X.boot,delta.boot,z1.boot, z2.boot,z3.boot, t0, FALSE),silent=TRUE)
        if(class(zanshi)=="try-error") next
        boot.NRI = rbind(boot.NRI,t(zanshi))
      }
      jack.NRI = NULL
...
#################################################################################
nSim <- 1000
NRIGerdsUsingFine400_NULL <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIGerdsUsingFine400_NULL,file = 'NRIGerdsUsingFine400_NULL.rda')
