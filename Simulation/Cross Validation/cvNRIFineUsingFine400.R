...
wrapper <- function(name){
  ...
  estNRI_Fine = function(Time,Event, Z1, Z2, Z3, t0, withsd)
  {
    ndim = length(Time)
    npts = length(t0)
    ...
    for(layer in cv){
      ...
      pred1 = predict(crr1,cbind(Z1[layer],Z2[layer]))
      # summary(crr1)
      ...
      p1.hat.m1 = rbind(p1.hat.m1,t(pred1[t0.index,-1]))
      
      crr2 = crr(X,delta,cov,failcode=2)
      pred2 = predict(crr2,cbind(Z1[layer],Z2[layer]))
      # summary(crr1)
      ...
      # estimate the overall survival function #
      p2.hat.m1 = rbind(p2.hat.m1,t(pred2[t0.index,-1]))
      ...
      cov = cbind(z1,z2,z3)
      crr1 = crr(X,delta,cov)
      pred1 = predict(crr1,cbind(Z1[layer],Z2[layer],Z3[layer]))
      # summary(crr1)
      ...
      # estimate the overall survival function #
      p1.hat.m2 = rbind(p1.hat.m2,t(pred1[t0.index,-1]))
      
      crr2 = crr(X,delta,cov,failcode=2)
      pred2 = predict(crr2,cbind(Z1[layer],Z2[layer],Z3[layer]))
      # summary(crr1)
      ...
      # estimate the overall survival function #
      p2.hat.m2 = rbind(p2.hat.m2,t(pred2[t0.index,-1]))
    }
...
for(ww in c(3,5)){
  ...
  for ( m in 1:nsim)	# number of simulations
  {
    ...
    cov1=ifelse(quantile(boot.NRI[,1],alpha1[1])<0.1135985 & quantile(boot.NRI[,1],alpha2[1])>0.1135985,1,0)
    cov2=ifelse(quantile(boot.NRI[,2],alpha1[2])<0.1275892 & quantile(boot.NRI[,2],alpha2[2])>0.1275892,1,0)
    cov3=ifelse(quantile(boot.NRI[,3],alpha1[3])<0.1243692 & quantile(boot.NRI[,3],alpha2[3])>0.1243692,1,0)
    
    cov10=ifelse(quantile(boot.NRI[,1],0.025)<0.1135985 & quantile(boot.NRI[,1],0.975)>0.1135985,1,0)
    cov20=ifelse(quantile(boot.NRI[,2],0.025)<0.1275892 & quantile(boot.NRI[,2],0.975)>0.1275892,1,0)
    cov30=ifelse(quantile(boot.NRI[,3],0.025)<0.1243692 & quantile(boot.NRI[,3],0.975)>0.1243692,1,0)
    
    cov11=ifelse(NRI[1]-qnorm(0.975)*sd.NRI[1]<0.1135985 & NRI[1]+qnorm(0.975)*sd.NRI[1]>0.1135985,1,0)
    cov21=ifelse(NRI[2]-qnorm(0.975)*sd.NRI[2]<0.1275892 & NRI[2]+qnorm(0.975)*sd.NRI[2]>0.1275892,1,0)
    cov31=ifelse(NRI[3]-qnorm(0.975)*sd.NRI[3]<0.1243692 & NRI[3]+qnorm(0.975)*sd.NRI[3]>0.1243692,1,0)
    
    cov12=ifelse(2*NRI[1]-quantile(boot.NRI[,1],0.975)<0.1135985 & 2*NRI[1]-quantile(boot.NRI[,1],0.025)>0.1135985,1,0)
    cov22=ifelse(2*NRI[2]-quantile(boot.NRI[,2],0.975)<0.1275892 & 2*NRI[2]-quantile(boot.NRI[,2],0.025)>0.1275892,1,0)
    cov32=ifelse(2*NRI[3]-quantile(boot.NRI[,3],0.975)<0.1243692 & 2*NRI[3]-quantile(boot.NRI[,3],0.025)>0.1243692,1,0)
...
#################################################################################
nSim <- 1000
NRIFineUsingFine400_Alternative_CV <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIFineUsingFine400_Alternative_CV,file = 'NRIFineUsingFine400_Alternative_CV.rda')
