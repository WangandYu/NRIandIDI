...
for(ww in c(3,5)){
  ...
  for ( m in 1:nsim)	# number of simulations
  {
    ...
    cov1=ifelse(quantile(boot.NRI[,1],alpha1[1])<0.10694095 & quantile(boot.NRI[,1],alpha2[1])>0.10694095,1,0)
    cov2=ifelse(quantile(boot.NRI[,2],alpha1[2])<0.09888901 & quantile(boot.NRI[,2],alpha2[2])>0.09888901,1,0)
    cov3=ifelse(quantile(boot.NRI[,3],alpha1[3])<0.12755996 & quantile(boot.NRI[,3],alpha2[3])>0.12755996,1,0)
    
    cov10=ifelse(quantile(boot.NRI[,1],0.025)<0.10694095 & quantile(boot.NRI[,1],0.975)>0.10694095,1,0)
    cov20=ifelse(quantile(boot.NRI[,2],0.025)<0.09888901 & quantile(boot.NRI[,2],0.975)>0.09888901,1,0)
    cov30=ifelse(quantile(boot.NRI[,3],0.025)<0.12755996 & quantile(boot.NRI[,3],0.975)>0.12755996,1,0)
    
    cov11=ifelse(NRI[1]-qnorm(0.975)*sd.NRI[1]<0.10694095 & NRI[1]+qnorm(0.975)*sd.NRI[1]>0.10694095,1,0)
    cov21=ifelse(NRI[2]-qnorm(0.975)*sd.NRI[2]<0.09888901 & NRI[2]+qnorm(0.975)*sd.NRI[2]>0.09888901,1,0)
    cov31=ifelse(NRI[3]-qnorm(0.975)*sd.NRI[3]<0.12755996 & NRI[3]+qnorm(0.975)*sd.NRI[3]>0.12755996,1,0)
    
    cov12=ifelse(2*NRI[1]-quantile(boot.NRI[,1],0.975)<0.10694095 & 2*NRI[1]-quantile(boot.NRI[,1],0.025)>0.10694095,1,0)
    cov22=ifelse(2*NRI[2]-quantile(boot.NRI[,2],0.975)<0.09888901 & 2*NRI[2]-quantile(boot.NRI[,2],0.025)>0.09888901,1,0)
    cov32=ifelse(2*NRI[3]-quantile(boot.NRI[,3],0.975)<0.12755996 & 2*NRI[3]-quantile(boot.NRI[,3],0.025)>0.12755996,1,0)
...
#################################################################################
nSim <- 1000
NRIWBUsingCox400_Alternative_CV <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIWBUsingCox400_Alternative_CV,file = 'NRIWBUsingCox400_Alternative_CV.rda')
