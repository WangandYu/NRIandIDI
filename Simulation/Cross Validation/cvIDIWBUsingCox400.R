...
wrapper <- function(name){
  ...
  estIDI_Weibull = function(Time,Event, Z1, Z2, Z3, t0, withsd)
  {
    ndim = length(Time)
    npts = length(t0)
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      cov1=ifelse(quantile(boot.IDI[,1],alpha1[1])<0.10883072 & quantile(boot.IDI[,1],alpha2[1])>0.10883072,1,0)
      cov2=ifelse(quantile(boot.IDI[,2],alpha1[2])<0.10666418 & quantile(boot.IDI[,2],alpha2[2])>0.10666418,1,0)
      cov3=ifelse(quantile(boot.IDI[,3],alpha1[3])<0.09872651 & quantile(boot.IDI[,3],alpha2[3])>0.09872651,1,0)
      cov10=ifelse(quantile(boot.IDI[,1],0.025)<0.10883072 & quantile(boot.IDI[,1],0.975)>0.10883072,1,0)
      cov20=ifelse(quantile(boot.IDI[,2],0.025)<0.10666418 & quantile(boot.IDI[,2],0.975)>0.10666418,1,0)
      cov30=ifelse(quantile(boot.IDI[,3],0.025)<0.09872651 & quantile(boot.IDI[,3],0.975)>0.09872651,1,0)
...
#################################################################################
nSim <- 1000
IDIWBUsingCox400_Alternative_CV <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(IDIWBUsingCox400_Alternative_CV,file = 'IDIWBUsingCox400_Alternative_CV.rda')
