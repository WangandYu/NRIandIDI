...
wrapper <- function(name){
  ...
  for(ww in c(3,5)){
    ...
    for ( m in 1:nsim)	# number of simulations
    {
      ...
      bias=NRI-NRI_zheng
      covs=(NRI+qnorm(0.025)*sd.NRI-bias < c(0.109168,0.1268454,0.1225807))&(NRI+qnorm(0.975)*sd.NRI-bias > c(0.109168,0.1268454,0.1225807))
...
#################################################################################
nSim <- 1000
NRIFineUsingFine400_Alternative_zheng <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(NRIFineUsingFine400_Alternative_zheng,file = 'NRIFineUsingFine400_Alternative_zheng.rda')
